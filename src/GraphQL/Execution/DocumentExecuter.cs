...
namespace GraphQL
{
    ...
    public class DocumentExecuter : IDocumentExecuter
    {
        ...
        public object GetVariableValue(ISchema schema, VariableDefinition variable, IValue input)
        {
            ...
            if (val == null)
            {
                throw new InvalidOperationException("Variable '${0}' of required type '{1}' was not provided.".ToFormat(variable.Name, type.Name ?? variable.Type.FullName()));
            }

            throw new InvalidOperationException("Variable '${0}' expected value of type '{1}'.".ToFormat(variable.Name, type?.Name ?? variable.Type.FullName()));
        }
        ...
        public bool IsValidValue(ISchema schema, GraphType type, IType astType, object input)
        {
            if (type is NonNullGraphType)
            {
                ...
                return IsValidValue(schema, schema.FindType(((NonNullGraphType)type).Type), astType, input);
            }
            ...
            if (type is ListGraphType)
            {
                ...
                return list != null && !(input is string)
                    ? list.All(item => IsValidValue(schema, listItemType, astType, item))
                    : IsValidValue(schema, listItemType, astType, input);
            }
            ...
            if (type is ScalarGraphType)
            {
                ...
                if (input is IValue)
                {
                    return scalar.ParseLiteral((IValue)input) != null;
                }

                return scalar.ParseValue(input) != null;
            }
